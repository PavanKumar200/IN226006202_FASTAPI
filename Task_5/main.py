...
#--search+sort+Pagination in one endpoint---
...
def browse_products(
    ...
    # Step 1: Search 
    result = products 
    if keyword: result = [p for p in result if keyword.lower() in p['name'].lower()] 
    # Step 2: Sort 
    if sort_by in ['price', 'name']: result = sorted(result, key=lambda p: p[sort_by], reverse=(order=='desc')) 
    # Step 3: Paginate 
    total = len(result) 
    start = (page - 1) * limit 
...
def search_orders(
    customer_name: str = Query(..., description="Search orders")
    ):
    results = [ o for o in orders 
               if customer_name.lower() in o['customer_name'].lower()]
    if not results:
        return{f"No orders found with customer name '{customer_name}' "}
    return{
